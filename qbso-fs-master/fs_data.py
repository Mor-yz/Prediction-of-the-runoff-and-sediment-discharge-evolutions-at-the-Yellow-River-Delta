...
class FSData():
    ...
    def __init__(self, typeOfAlgo, location, nbr_exec, method, test_param, param, val, classifier, alpha=None,
        ...
        self.ql = QLearning(len(self.df.columns), Solution.attributs_to_flip(len(self.df.columns) - 1), alpha, gamma,
                            epsilon)
        ...
        self.instance_name = self.dataset_name + '_' + str(
            time.strftime("%m-%d-%Y_%H-%M-%S_", time.localtime()) + self.classifier_name)
    ...
    def run(self, flip, max_chance, bees_number, maxIterations, locIterations):
        ...
        for itr in range(1, self.nb_exec + 1):
            ...
            tmp1 = [i for i in swarm.best_features()]
            self.worksheet.write(itr, 4, "{0}".format(str([j[0] for j in tmp1])))
            self.worksheet.write(itr, 5, len(Solution.solutions))
            self.worksheet.write(itr, 6, str(swarm.bestSolution.solution.get_state()))
            # iteration_['Iteration'].append(itr)
            # iteration_['Accuracy'].append("{0:.2f}".format(best[0]))
            # iteration_['N_Features'].append(best[1])

        print("Total execution time of {0} executions \nfor dataset \"{1}\" is {2:.2f} s".format(self.nb_exec,
                                                                                                 self.dataset_name,
                                                                                                 total_time))
        self.workbook.close()
