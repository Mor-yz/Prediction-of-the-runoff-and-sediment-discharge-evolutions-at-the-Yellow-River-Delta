...
class Solution:
    ...
    def set_accuracy(self, state):
        t1 = time.time()
        Solution.solutions[Solution.str_sol(state)] = self.data.evaluate2(state)
        self.accuracy = Solution.solutions[Solution.str_sol(state)]
        t2 = time.time()
        Solution.tot_eval_time += t2 - t1
        if Solution.best_sol is None or (
                Solution.best_sol.get_accuracy(Solution.best_sol.get_state()) < self.accuracy):
